...
SSA.prototype.buildScope = function buildScope(node) {
  ...
  if (isFn) {
    ...
    if (name) {
      var self;
      if (isRoot) {
        assert(this.parent, 'Named function without parent!');
        var slot = this.parent.pendingSlots.shift();
        assert(slot, 'Named function without pending slot!');

        self = new Slot(slot);
      } else if (node.type === 'FunctionDeclaration') {
        // TODO(indutny): do it only when the function is using it's name
        self = new Slot();
        self.toContext(this);
        self.initial = node;
        this.pendingSlots.push(self);

        // Do not store slot in scope
