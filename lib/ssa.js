...
function SSA(ast, options, parent) {
  this.blockId = 0;
  this.valueId = 0;
  this.astId = 0;
...
SSA.construct = function construct(ast, options) {
  return new SSA(ast, options).construct();
};
...
SSA.prototype.buildScope = function buildScope(node) {
  ...
  if (isFn) {
    ...
    for (var i = 0; i < node.declarations.length; i++) {
      ...
      if (this.ast.type === 'Program')
        slot.type = 'global';
      this.scope[node.declarations[i].id.name] = slot;
    }
  }
...
SSA.prototype.visitExpr = function visitExpr(node) {
...
SSA.prototype.visitFunction = function visitFunction(node) {
  ...
  var fn = this.add('fn', [ this.createValue('js', ssa[0].id) ]);
  return fn;
};
...
SSA.prototype.createChild = function createChild(ast) {
  return new SSA(ast, this.options, this.root);
};
...
function Value(ssa, type, subtype, inputs) {
  ...
  this.ast = ssa.root.currentAst;
  this.astId = ssa.root.currentAstId;
